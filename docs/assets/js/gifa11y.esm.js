...
function toggleAll(newState = "detect") {
  ...
  if (pageState === "paused") {
    ...
    window.gifa11yOption.initiallyPaused = true;
  } else {
    playDisplay = "none";
    pauseDisplay = "block";
...
function everythingToggle() {
  ...
  if (everythingButton !== null) {
    ...
    if (notReady.length > 0) {
      const promises = window.a11ygifs.filter((image) => !image.complete).map(
...
function findGifs($newGifs, option) {
  ...
  const images = Array.from(
    container.querySelectorAll(
      `:is(${option.target}):not([src*="gifa11y-ignore"], [data-gifa11y-state], .gifa11y-ignore${exclusions})`
    )
  );
  images.forEach(($gif) => {
    $newGifs.push($gif);
...
function generateButtons(gif, option) {
  ...
  if (!mediaQuery || mediaQuery.matches || image.classList.contains("gifa11y-paused") || filename.indexOf("gifa11y-paused") > -1 || option.initiallyPaused === true) {
...
function generateStill(gif, option) {
  ...
  const mediaQuery = window.matchMedia("(prefers-reduced-motion: reduce)");
  const shouldPause = mediaQuery.matches || image.classList.contains("gifa11y-paused") || image.src.includes("gifa11y-paused") || option.initiallyPaused;
